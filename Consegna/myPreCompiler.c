...
// Fasi di elaborazione del file
typedef enum { GLOBAL_DECLS, LOCAL_DECLS, BODY } Phase;

// Inizializza la struttura per le statistiche e gli errori
...
// Libera la memoria allocata nella struttura delle statistiche
...
// Aggiunge un errore alla struttura delle statistiche, con conteggio per tipo di errore
...
// Aggiunge una variabile alla struttura delle statistiche
...
// Aggiunge un typedef alla struttura delle statistiche
...
// Controlla se un nome è presente tra i typedef raccolti
...
// Non include typedef definiti dall'utente, che vengono gestiti separatamente.
...
// Controlla se una stringa è un identificatore C valido (per nomi di variabili o typedef)
...
static bool is_declaration_line(Stats *stats, const char *cleaned) {
    ...
    if (strcmp(tok1, "typedef") == 0 || is_type_keyword(tok1) ||
        ...
        return true;
    // Parole chiave che indicano controllo di flusso o chiamate a funzione non sono dichiarazioni
    ...
    // La seconda parola deve essere un identificatore valido (nome variabile) o un puntatore (es. "int *ptr;")
...
// Elabora un file di input, aggiornando le statistiche e gli errori rilevati
void process_file(FILE *in_fp, const char *filename, Stats *stats) {
    ...
    while (fgets(line, sizeof(line), in_fp)) {
        ...
        // Se troviamo una graffa di chiusura isolata, usata per chiudere un blocco di struct, la gestiamo qui e saltiamo la riga
        ...
        if (strncmp(cleaned, "typedef", 7) == 0) {
            ...
            /* typedef semplice (incluso "typedef struct Tag Alias;") dentro un blocco*/
            {
                char temp_td[MAX_LINE_LEN];
                strcpy(temp_td, cleaned);
                char *p    = temp_td + 7;
        ...
        if (phase == GLOBAL_DECLS || phase == LOCAL_DECLS) {
            ...
            if (phase != BODY) {
                ...
                while (*ptr) {
                    ...
                    // Estraiamo la parola successiva (tipo o parte di esso, es. "unsigned" o "struct Point")
                    ...
                    word[i] = '\0';
                    // Se la parola inizia con '*', la consideriamo parte del tipo (es. "int *ptr;")
                    if (i == 0) {
                        if (*ptr == '*') {
                    ...
                    // Controlliamo se la parola è una keyword di tipo o un typedef definito dall'utente
                    ...
                    // Se la parola non è un tipo valido, consideriamo l'intera riga come tipo errato
                    } else {
                        if (type[0] == '\0') {
                            strcpy(type, word);
                            type_ok = false;
                ...
                // Ora "ptr" punta alla parte della riga dopo il tipo, che dovrebbe contenere i nomi delle variabili dichiarate
                char *v_tok = strtok(ptr, ",;");
                while (v_tok) {
                    char *start = v_tok;
                    while (*start == ' ' || *start == '\t' || *start == '*') start++;
                    // Rimuoviamo eventuali array o inizializzatori (es. "arr[10]", "x = 5") per isolare il nome della variabile
                    for (int k = 0; start[k]; k++) {
                        if (start[k] == '=' || start[k] == '[') {
                            start[k] = '\0'; break;
                        }
                    }
                    // Rimuoviamo spazi finali dal nome della variabile
                    char *end = start + strlen(start) - 1;
                    while (end > start && (*end == ' ' || *end == '\t')) *end-- = '\0';
                    /* Se non è un valido identificatore, segnala errore, comunque aggiungiamo la variabile
                     * (anche se con nome errato) per continuare a tracciare eventuali usi successivi
                     */
                    if (strlen(start) > 0) {
...
// Stampa le statistiche raccolte e i dettagli degli errori, su file di output e/o stdout in base alla modalità verbose
