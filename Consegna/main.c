...
#include <getopt.h> // Per getopt_long e struct option

#define MAX_FILES 50

// In caso di errore stampa l'uso del programma
...
int main(int argc, char *argv[]) {
    ...
    // getopt_long restituisce -1 quando non ci sono più opzioni da processare
    int opt;
    ...
    // Controllo che sia stato specificato almeno un file di input
    if (file_count == 0) {
    ...
    // Apertura file di output se specificato, altrimenti usiamo stdout
    FILE *out_fp = stdout;
    if (output_file) {
    ...
    // Inizializzazione struttura per statistiche ed errori
    Stats stats;
    init_stats(&stats);

    // Elaborazione di ogni file di input
    for (int i = 0; i < file_count; i++) {
        ...
        // Controllo se il file è vuoto o non leggibile
    ...
    // Stampa delle statistiche e degli errori raccolti e rilascio risorse
    print_statistics(out_fp, &stats, verbose);
    free_stats(&stats);

    // Chiusura file di output se era stato aperto
    if (out_fp != stdout) {
        if (fclose(out_fp) != 0) {
